"""Main experiment window for the 3D inverse MDS task."""
...
from PySide6.QtGui import QCursor, QDesktopServices, QFont, QKeySequence, QPixmap, QShortcut, QVector3D
...
from pyqtgraph.opengl import GLGridItem, GLLinePlotItem

from core.export import export_results_csv
...
class ExperimentWindow(QMainWindow):
    ...
    def _build_cube_wireframe(length: float):
        ...
        for y in vals:
            for z in vals:
                segments.append(((0, y, z), (length, y, z)))
        for x in vals:
            for z in vals:
                segments.append(((x, 0, z), (x, length, z)))
        for x in vals:
            for y in vals:
                segments.append(((x, y, 0), (x, y, length)))
        return [_line_segments_item(segments, color=CUBE_COLOR, width=CUBE_WIDTH)]
    ...
    def _build_lattice_grid(length: float, step: float):
        ...
        for x in ticks:
            for y in ticks:
                segments.append(((x, y, 0), (x, y, length)))
        return [_line_segments_item(segments, color=LATTICE_COLOR, width=LATTICE_WIDTH)]
    ...
    def update_pair_line(self, cat: str):
        ...
        if not self._debug_on():
            if cat in self.pair_lines:
                ...
                try:
                    ...
                    pass
                try:
                    line.setData(pos=np.empty((0, 3), dtype=np.float32))
                except Exception:
                    pass
        ...
        if has1 and has2:
            ...
            pts = np.ascontiguousarray([c1, c2], dtype=np.float32)
            if cat in self.pair_lines:
                self.pair_lines[cat].setData(pos=pts)
            else:
                line = _line_item(c1, c2, color=(1, 1, 1, 1), width=1)
    ...
    def export_results(self):
        """Export placed points to CSV and advance to the next configured condition."""
        if export_results_csv(self) is None:
            return

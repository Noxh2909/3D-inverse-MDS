...
def _make_abbrev(full_name: str) -> str:
    ...
    "First Last" -> "F.L" (first letter of each word joined by ".").
    Falls back to the raw name if it cannot be split.
    """
...
class Logger:
    ...
    def log_session_event(self, event: str):
        ...
        if self._name_provider:
            ...
            name = _make_abbrev(raw) if raw != "anonymous" else "anonymous"
...
class ExperimentWindow(QMainWindow):
    ...
    def export_results(self):
        ...
        raw_name = self.name_input.text().strip() or "anonymous"
        participant_name = _make_abbrev(raw_name) if raw_name != "anonymous" else "anonymous"
